...
std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> Point2VoxelCPU::point_to_voxel_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor densehashdata, tv::Tensor points_voxel_id, std::array<float, 3> vsize, std::array<int, 3> grid_size, std::array<int, 3> grid_stride, std::array<float, 6> coors_range, bool clear_voxels)   {
  ...
  tv::dispatch<float, double>(points.dtype(), [&](auto I){
      ...
      int coor[3];
      auto coor_to_voxelidx_rw = densehashdata.tview<int, 3>();
      int voxelidx, num;
      bool failed;
      int voxel_num = 0;
      for (int i = 0; i < N; ++i) {
          failed = false;
          for (int j = 0; j < 3; ++j) {
              c = floor((points_rw(i, 2 - j) - coors_range[j]) / vsize[j]);
