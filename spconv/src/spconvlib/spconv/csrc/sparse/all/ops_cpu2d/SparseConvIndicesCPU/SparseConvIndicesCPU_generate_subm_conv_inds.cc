...
int SparseConvIndicesCPU::generate_subm_conv_inds(tv::Tensor indices, tv::Tensor indice_pairs, tv::Tensor out_inds, tv::Tensor indice_num_per_loc, int batch_size, tv::array<int, 2> input_dims, tv::array<int, 2> ksize, tv::array<int, 2> dilation)   {
  
  tv::array<int, 2> stride, padding;
  for (int i = 0; i < 2; ++i){
