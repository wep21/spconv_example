...
std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> Point2VoxelCPU::point_to_voxel_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor densehashdata, tv::Tensor points_voxel_id, std::array<float, 1> vsize, std::array<int, 1> grid_size, std::array<int, 1> grid_stride, std::array<float, 2> coors_range, bool clear_voxels)   {
  ...
  tv::dispatch<float, double>(points.dtype(), [&](auto I){
      ...
      int coor[1];
      auto coor_to_voxelidx_rw = densehashdata.tview<int, 1>();
      int voxelidx, num;
      bool failed;
      int voxel_num = 0;
      for (int i = 0; i < N; ++i) {
          failed = false;
          for (int j = 0; j < 1; ++j) {
              c = floor((points_rw(i, 0 - j) - coors_range[j]) / vsize[j]);
          ...
          voxelidx = coor_to_voxelidx_rw(coor[0]);
          if (voxelidx == -1) {
              voxelidx = voxel_num;
              ...
              voxel_num += 1;
              coor_to_voxelidx_rw(coor[0]) = voxelidx;
              for (int k = 0; k < 1; ++k) {
      ...
      for (int i = 0; i < voxel_num; ++i) {
          coor_to_voxelidx_rw(coors_rw(i, 0)) = -1;
