...
std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> Point2Voxel::point_to_voxel_hash_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor hashdata, tv::Tensor point_indice_data, tv::Tensor points_voxel_id, std::array<float, 4> vsize, std::array<int, 4> grid_size, std::array<int64_t, 4> grid_stride, std::array<float, 8> coors_range, bool clear_voxels, bool empty_mean, std::uintptr_t stream_int)   {
  ...
  TV_ASSERT_INVALID_ARG(points.ndim() == 2 && points.dim(1) >= 4, "error");
  using V = int64_t;
