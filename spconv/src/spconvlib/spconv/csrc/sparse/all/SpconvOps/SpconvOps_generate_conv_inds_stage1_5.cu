...
int SpconvOps::generate_conv_inds_stage1_5(tv::Tensor indice_pairs_uniq, int ndim, int64_t uniq_size, std::uintptr_t stream_int)   {
  
  if (ndim == 1){
      return SpconvIndices1D::generate_conv_inds_stage1_5(indice_pairs_uniq, uniq_size, stream_int);
  }
  if (ndim == 2){
      return SpconvIndices2D::generate_conv_inds_stage1_5(indice_pairs_uniq, uniq_size, stream_int);
  }
  if (ndim == 3){
      return SpconvIndices3D::generate_conv_inds_stage1_5(indice_pairs_uniq, uniq_size, stream_int);
  }
  if (ndim == 4){
      return SpconvIndices4D::generate_conv_inds_stage1_5(indice_pairs_uniq, uniq_size, stream_int);
  }
