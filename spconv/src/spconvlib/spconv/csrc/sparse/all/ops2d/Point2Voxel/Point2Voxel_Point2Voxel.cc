...
using Layout = spconvlib::spconv::csrc::sparse::all::ops2d::layout_ns::TensorGeneric;
 Point2Voxel::Point2Voxel(std::array<float, 2> vsize_xyz, std::array<float, 4> coors_range_xyz, int num_point_features, int max_num_voxels, int max_num_points_per_voxel)   {
  ...
  indices = tv::zeros({max_num_voxels, 2}, tv::int32, 0);
