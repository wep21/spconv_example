...
std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> Point2VoxelCPU::point_to_voxel_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor densehashdata, tv::Tensor points_voxel_id, std::array<float, 4> vsize, std::array<int, 4> grid_size, std::array<int, 4> grid_stride, std::array<float, 8> coors_range, bool clear_voxels)   {
  ...
  tv::dispatch<float, double>(points.dtype(), [&](auto I){
      ...
      int coor[4];
      auto coor_to_voxelidx_rw = densehashdata.tview<int, 4>();
      int voxelidx, num;
      bool failed;
      int voxel_num = 0;
      for (int i = 0; i < N; ++i) {
          failed = false;
          for (int j = 0; j < 4; ++j) {
              c = floor((points_rw(i, 3 - j) - coors_range[j]) / vsize[j]);
          ...
          voxelidx = coor_to_voxelidx_rw(coor[0], coor[1], coor[2], coor[3]);
          if (voxelidx == -1) {
              voxelidx = voxel_num;
              ...
              voxel_num += 1;
              coor_to_voxelidx_rw(coor[0], coor[1], coor[2], coor[3]) = voxelidx;
              for (int k = 0; k < 4; ++k) {
      ...
      for (int i = 0; i < voxel_num; ++i) {
          coor_to_voxelidx_rw(coors_rw(i, 0), coors_rw(i, 1), coors_rw(i, 2), coors_rw(i, 3)) = -1;
