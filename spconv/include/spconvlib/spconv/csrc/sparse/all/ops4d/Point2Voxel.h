...
#include <spconvlib/spconv/csrc/sparse/all/ops4d/layout_ns/TensorGeneric.h>
namespace spconvlib {
namespace spconv {
namespace csrc {
namespace sparse {
...
struct Point2Voxel {
  ...
  tv::array<int64_t, 4> grid_stride;
   ...
   */
   Point2Voxel(std::array<float, 4> vsize_xyz, std::array<float, 8> coors_range_xyz, int num_point_features, int max_num_voxels, int max_num_points_per_voxel);
  /**
   * @param points 
   * @param clear_voxels 
   * @param empty_mean 
   ...
   * @param stream_int 
   */
  static std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> point_to_voxel_hash_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor hashdata, tv::Tensor point_indice_data, tv::Tensor points_voxel_id, std::array<float, 4> vsize, std::array<int, 4> grid_size, std::array<int64_t, 4> grid_stride, std::array<float, 8> coors_range, bool clear_voxels = true, bool empty_mean = false, std::uintptr_t stream_int = 0);
};
} // namespace ops4d
} // namespace all
} // namespace sparse
