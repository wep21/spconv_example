...
__global__ void build_hash_table(TTable table, float const* points, int64_t * points_indice_data, int point_stride, tv::array<float, 2> vsize, tv::array<float, 4> coors_range, tv::array<int, 2> grid_bound, tv::array<int64_t, 2> grid_stride, int num_points)   {
  ...
  #pragma unroll
      for (int j = 0; j < 2; ++j) {
          c = floor((points[i * point_stride + 1 - j] - coors_range[j]) /
                      vsize[j]);
  ...
  for (int i : tv::KernelLoopX<int>(table.size())){
      ...
      if (!item.empty()) {
          ...
          if (item.second < max_voxels){
              layout.inverse(item.first, indices + item.second * 2);
          }
      }
  }
}
template <typename TTable>
__global__ void generate_voxel(TTable table, float const* points, const int64_t* points_indice_data, float * voxels, int * num_per_voxel, int64_t* points_voxel_id, int point_stride, int max_points_per_voxel, int max_voxels, tv::array<float, 2> vsize, tv::array<float, 4> coors_range, tv::array<int, 2> grid_bound, tv::array<int64_t, 2> grid_stride, int num_points)   {
