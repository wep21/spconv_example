...
#include <spconvlib/spconv/csrc/sparse/all/ops1d/p2v_c/Point2VoxelCommon.h>
...
struct Point2Voxel {
  ...
  tv::array<float, 1> vsize;
  tv::array<float, 2> coors_range;
  tv::array<int, 1> grid_size;
  tv::array<int64_t, 1> grid_stride;
  
  std::array<int, 1> get_grid_size();
  /**
   * @param vsize_xyz 
   ...
   */
   Point2Voxel(std::array<float, 1> vsize_xyz, std::array<float, 2> coors_range_xyz, int num_point_features, int max_num_voxels, int max_num_points_per_voxel);
  /**
   * @param points 
   * @param clear_voxels 
   * @param empty_mean 
   ...
   * @param stream_int 
   */
  static std::tuple<tv::Tensor, tv::Tensor, tv::Tensor> point_to_voxel_hash_static(tv::Tensor points, tv::Tensor voxels, tv::Tensor indices, tv::Tensor num_per_voxel, tv::Tensor hashdata, tv::Tensor point_indice_data, tv::Tensor points_voxel_id, std::array<float, 1> vsize, std::array<int, 1> grid_size, std::array<int64_t, 1> grid_stride, std::array<float, 2> coors_range, bool clear_voxels = true, bool empty_mean = false, std::uintptr_t stream_int = 0);
};
} // namespace ops1d
} // namespace all
} // namespace sparse
