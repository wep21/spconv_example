...
struct TensorGeneric {
  tv::array<int64_t, 1> strides;
  TV_HOST_DEVICE_INLINE  TensorGeneric(tv::array<int64_t, 1> const& strides) : strides(strides)  {
  ...
  TV_HOST_DEVICE_INLINE int64_t operator()(const tv::array<int, 2> & indexes)  const {
    ...
    return indexes[1] + int64_t(strides[0] * int64_t(indexes[0]));
  }
