...
struct ConvProblem {
  ...
  int C;
  int K;
  tv::array<int, 2> input_dims;
  tv::array<int, 2> output_dims;
  tv::array<int, 2> ksize;
  tv::array<int, 2> padding;
  tv::array<int, 2> stride;
  tv::array<int, 2> dilation;
  ...
  TV_HOST_DEVICE_INLINE tv::array<int, 3> get_npq_shape()   {
    
    return {N, output_dims[0], output_dims[1]};
  }
  TV_HOST_DEVICE_INLINE bool check_npq_not_overflow()   {
    
    auto shape = get_npq_shape();
    return (
    std::abs(int64_t(shape[0]) * int64_t(shape[1]) * int64_t(shape[2])) <= std::numeric_limits<int>::max()
    );
  }
  TV_HOST_DEVICE_INLINE static tv::array<int, 2> calc_output_dims(tv::array<int, 2> input_dims, tv::array<int, 2> ksize, tv::array<int, 2> padding, tv::array<int, 2> stride, tv::array<int, 2> dilation)   {
  ...
  TV_HOST_DEVICE_INLINE tv::array<int, 4> get_input_shape()   {
    
    return {N, input_dims[0], input_dims[1], C};
  }
  TV_HOST_DEVICE_INLINE tv::array<int, 4> get_weight_shape()   {
    
    return {K, ksize[0], ksize[1], C};
  }
  TV_HOST_DEVICE_INLINE tv::array<int, 4> get_output_shape()   {
    
    return {N, output_dims[0], output_dims[1], K};
  }
};
} // namespace spinds
} // namespace ops_cpu2d
