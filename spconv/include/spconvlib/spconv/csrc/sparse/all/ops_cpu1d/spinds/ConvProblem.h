...
struct ConvProblem {
  ...
  int C;
  int K;
  tv::array<int, 1> input_dims;
  tv::array<int, 1> output_dims;
  tv::array<int, 1> ksize;
  tv::array<int, 1> padding;
  tv::array<int, 1> stride;
  tv::array<int, 1> dilation;
  ...
  TV_HOST_DEVICE_INLINE tv::array<int, 2> get_npq_shape()   {
    
    return {N, output_dims[0]};
  }
  TV_HOST_DEVICE_INLINE bool check_npq_not_overflow()   {
    
    auto shape = get_npq_shape();
    return (
    std::abs(int64_t(shape[0]) * int64_t(shape[1])) <= std::numeric_limits<int>::max()
    );
  }
  TV_HOST_DEVICE_INLINE static tv::array<int, 1> calc_output_dims(tv::array<int, 1> input_dims, tv::array<int, 1> ksize, tv::array<int, 1> padding, tv::array<int, 1> stride, tv::array<int, 1> dilation)   {
  ...
  TV_HOST_DEVICE_INLINE tv::array<int, 3> get_input_shape()   {
    
    return {N, input_dims[0], C};
  }
  TV_HOST_DEVICE_INLINE tv::array<int, 3> get_weight_shape()   {
    
    return {K, ksize[0], C};
  }
  TV_HOST_DEVICE_INLINE tv::array<int, 3> get_output_shape()   {
    
    return {N, output_dims[0], K};
  }
};
} // namespace spinds
} // namespace ops_cpu1d
