...
struct TensorGeneric {
  tv::array<int64_t, 3> strides;
  TV_HOST_DEVICE_INLINE  TensorGeneric(tv::array<int64_t, 3> const& strides) : strides(strides)  {
    
  }
  TV_HOST_DEVICE_INLINE static TensorGeneric from_shape(const tv::array<int, 4> & shape)   {
    
    return TensorGeneric({
    int64_t(shape[1]) * int64_t(shape[2]) * int64_t(shape[3]),
  ...
  TV_HOST_DEVICE_INLINE int64_t operator()(const tv::array<int, 4> & indexes)  const {
    ...
    return indexes[3] + int64_t(strides[0] * int64_t(indexes[0])) + int64_t(strides[1] * int64_t(indexes[1])) + int64_t(strides[2] * int64_t(indexes[2]));
  }
