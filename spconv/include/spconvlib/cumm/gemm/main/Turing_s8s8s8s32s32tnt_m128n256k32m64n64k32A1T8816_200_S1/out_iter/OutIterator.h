...
#include <spconvlib/cumm/gemm/main/gpTuring_s8s8s8s32s32tnt_m128n256k32m64n64k32A1T8816_200_S1/out_params_ns/tmap/Out5DLinear.h>
...
using ThreadMap = spconvlib::cumm::gemm::main::gpTuring_s8s8s8s32s32tnt_m128n256k32m64n64k32A1T8816_200_S1::out_params_ns::tmap::Out5DLinear;
using Params = spconvlib::cumm::gemm::main::gpTuring_s8s8s8s32s32tnt_m128n256k32m64n64k32A1T8816_200_S1::out_params_ns::OutIteratorParams;
struct OutIterator {
  int8_t * pointer_;
  ...
  __forceinline__ __device__  OutIterator(Params const& params, int8_t * ptr, tv::array<int, 2> extent, tv::array<int, 2> offset_2d, int thread_idx) : params_(params)  {
    ...
    for (int c = 0; c < 1; ++c) {
        for (int v = 0; v < 1; ++v){
            column_masks_[c][v] = ((thread_offset[1] + 256 * c + v * 16) < extent[1]);
        }
    }
        ...
        for (int row = 0; row < 1; ++row){
          ...
          for (int column = 0; column < 1; ++column){
            ...
            tv::gemm::global_store<tv::alignedarray<int4, 1, 16>, sizeof(tv::alignedarray<int4, 1, 16>)>(
                ...
                (void *)&memory_pointer[column * 256 / 16],
                guard);
          }
        }
      }
        ...
        for (int row = 0; row < 1; ++row){
          ...
          for (int column = 0; column < 1; ++column){
            ...
            tv::gemm::global_load<tv::alignedarray<int4, 1, 16>, sizeof(tv::alignedarray<int4, 1, 16>)>(
                frag_ptr[frag_row_idx *  1 + column],
                (const void *)&memory_pointer[column * 256 / 16],
                guard);
          }
        }
      }
