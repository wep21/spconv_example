...
struct OutWarpTileIterator {
  ...
  __forceinline__ __device__  OutWarpTileIterator(int32_t * ptr, int warp_offset_m, int warp_offset_n, int lane_idx)   {
    ...
    pointer_ = reinterpret_cast<tv::alignedarray<int, 1, 4> *>(
        ptr + (logical_offset[0] * 8 + lane_offset_0) * 145 +
        logical_offset[1] * 32 +
        lane_offset_1 * 4);
  }
