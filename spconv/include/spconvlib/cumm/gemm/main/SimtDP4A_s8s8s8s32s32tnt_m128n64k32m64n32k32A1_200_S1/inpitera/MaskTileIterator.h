...
#include <spconvlib/cumm/gemm/main/SimtDP4A_s8s8s8s32s32tnt_m128n64k32m64n32k32A1_200_S1/inpitera/maskiter/PitchLinear.h>
...
#include <spconvlib/cumm/gemm/main/SimtDP4A_s8s8s8s32s32tnt_m128n64k32m64n32k32A1_200_S1/inpitera/maskiter/GlobalLoad.h>
namespace spconvlib {
namespace cumm {
namespace gemm {
namespace main {
...
using ThreadMap = spconvlib::cumm::gemm::main::SimtDP4A_s8s8s8s32s32tnt_m128n64k32m64n32k32A1_200_S1::inpitera::maskiter::PitchLinear;
using Params = spconvlib::cumm::gemm::main::gpSimtDP4A_s8s8s8s32s32tnt_m128n64k32m64n32k32A1_200_S1::itera_p::MaskTileIteratorParams;
using GlobalLoad = spconvlib::cumm::gemm::main::SimtDP4A_s8s8s8s32s32tnt_m128n64k32m64n32k32A1_200_S1::inpitera::maskiter::GlobalLoad;
struct MaskTileIterator {
  ...
  __forceinline__ __device__  MaskTileIterator(Params const & params, const int8_t * ptr, tv::array<int, 2> extent, int thread_id, const tv::array<int, 2>& threadblock_offset) : params_(params), pointer_(reinterpret_cast<const char *>(ptr)), extent_(extent), is_residue_tile_(true)  {
    ...
    // tv::printf2_once(params_.inc_strided_, params_.inc_next_);
        residue_offset_ = 
    (extent[1] - threadblock_offset[1] - 1) /
    32 * 32
    ;
  ...
  __forceinline__ __device__ void update_indices()   {
    ...
    for (int s = 0; s < 2; ++s){
        ...
        for (int ss = 0; ss < 4; ++ss){
            if (thread_offset_[0] + s * 64 + ss < extent_[0])
                indices_[s * 4 + ss] = 
  ...
  __forceinline__ __device__ void load_with_byte_offset(tv::array<int8_t, 32, 0>& frag, int64_t byte_offset)   {
