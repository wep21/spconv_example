...
struct SmemTileIteratorV2 {
  ...
  __forceinline__ __device__  SmemTileIteratorV2(int stride, float* ptr, int thread_id)   {
    ...
    int offset = (thread_offset[1] / 1) * 34
         + thread_offset[0] * 1;
  ...
  __forceinline__ __device__ void store_with_pointer_offset(tv::array<float, 2, 0> const & frag, int32_t pointer_offset)   {
