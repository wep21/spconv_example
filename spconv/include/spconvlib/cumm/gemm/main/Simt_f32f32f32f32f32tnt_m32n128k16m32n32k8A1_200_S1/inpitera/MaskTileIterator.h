...
#include <spconvlib/cumm/gemm/main/Simt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1/inpitera/maskiter/PitchLinear.h>
#include <spconvlib/cumm/gemm/main/gpSimt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1/itera_p/MaskTileIteratorParams.h>
#include <spconvlib/cumm/gemm/main/Simt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1/inpitera/maskiter/GlobalLoad.h>
namespace spconvlib {
namespace cumm {
namespace gemm {
namespace main {
...
using ThreadMap = spconvlib::cumm::gemm::main::Simt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1::inpitera::maskiter::PitchLinear;
using Params = spconvlib::cumm::gemm::main::gpSimt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1::itera_p::MaskTileIteratorParams;
using GlobalLoad = spconvlib::cumm::gemm::main::Simt_f32f32f32f32f32tnt_m32n128k16m32n32k8A1_200_S1::inpitera::maskiter::GlobalLoad;
struct MaskTileIterator {
  ...
  int32_t indices_[2];
  __forceinline__ __device__  MaskTileIterator(Params const & params, const float * ptr, tv::array<int, 2> extent, int thread_id, const tv::array<int, 2>& threadblock_offset) : params_(params), pointer_(reinterpret_cast<const char *>(ptr)), extent_(extent), is_residue_tile_(true)  {
  ...
  __forceinline__ __device__ void add_pointer_offset(int64_t offset)   {
    pointer_ += sizeof(float) * offset;
  }
  __forceinline__ __device__ void tile_increment(int num_tile)   {
    
    if (is_residue_tile_){
      thread_offset_[1] += residue_offset_;
      pointer_ += sizeof(float) * residue_offset_;
      compute_predicates_(extent_, true);
      pointer_ += 64 * (num_tile - 1);
    }
    else{
      pointer_ += 64 * num_tile;
    }
    is_residue_tile_ = false;
  }
  __forceinline__ __device__ const tv::alignedarray<int, 1, 4> * get(int s, int c, int v)  const {
  ...
  __forceinline__ __device__ void load_with_pointer_offset(tv::array<float, 2, 0>& frag, int32_t pointer_offset)   {
    load_with_byte_offset(frag, pointer_offset * sizeof(float));
  }
  ...
  __forceinline__ __device__ void load_with_byte_offset(tv::array<float, 2, 0>& frag, int64_t byte_offset)   {
  ...
  __forceinline__ __device__ void load(tv::array<float, 2, 0>& frag)   {
