...
#include <spconvlib/cumm/conv/main/Turing_s8s8s8s32f32tnt_m32n32k32m16n16k32A1T8816_200_C301LLL_SKD_S8/out_op/unaryop/UnaryActivation.h>
namespace spconvlib {
namespace cumm {
namespace conv {
namespace main {
...
namespace out_op {
using UnaryOp = spconvlib::cumm::conv::main::Turing_s8s8s8s32f32tnt_m32n32k32m16n16k32A1T8816_200_C301LLL_SKD_S8::out_op::unaryop::UnaryActivation;
struct Int8Inference {
  ...
  __forceinline__ __device__ tv::array<int8_t, 4, 0> operator()(tv::array<int32_t, 4, 0> const& accumulator, tv::array<int8_t, 4, 0> const& source, tv::array<float, 4, 0> const& bias, tv::array<float, 4, 0> const& scale)  const {
    
    tv::gemm::NumericArrayConverter<float, int8_t, 4, tv::gemm::FloatRoundStyle::round_to_nearest>
        source_converter;
    ...
    intermediate = op(intermediate, act_type, act_alpha, act_beta);
      ...
      tv::math::minimum<tv::array<float, 4, 0>> min_op;
      tv::math::maximum<tv::array<float, 4, 0>> max_op;
      intermediate = max_op(intermediate, -kClamp - float(1));
      intermediate = min_op(intermediate, kClamp);
    #endif
    // Convert to destination numeric type
    tv::gemm::NumericArrayConverter<int8_t, float, 4, tv::gemm::FloatRoundStyle::round_to_nearest>
        destination_converter;
    return destination_converter(intermediate);
  }
  __forceinline__ __device__ tv::array<int8_t, 4, 0> operator()(tv::array<int32_t, 4, 0> const& accumulator, tv::array<float, 4, 0> const& bias, tv::array<float, 4, 0> const& scale)  const {
