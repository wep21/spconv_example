...
#include <spconvlib/cumm/conv/main/Simt_f16f16f16f32f32tnt_m64n128k8m32n64k8A1_200_C301LLL_SK/output/out_ns_smem/tmap/Out5DLinear.h>
namespace spconvlib {
namespace cumm {
namespace conv {
namespace main {
...
using ThreadMap = spconvlib::cumm::conv::main::Simt_f16f16f16f32f32tnt_m64n128k8m32n64k8A1_200_C301LLL_SK::output::out_ns_smem::tmap::Out5DLinear;
struct OutSmemLoader {
  ...
  __forceinline__ __device__  OutSmemLoader(float * ptr, int thread_idx)   {
    ...
    pointer_ = ptr + thread_offset[0] * 161 + thread_offset[1];
  }
        ...
        for (int group = 0; group < 2; ++group) {
            ...
            for (int row = 0; row < 1; ++row) {
                const float * cur_pointer =
                    pointer_ + row * 1 * 161 + group * 2 * 161 +
                    cluster * 1 * 161 + pointer_offset;
                int frag_row_idx =
