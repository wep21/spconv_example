...
struct Out5DLinear {
  __forceinline__ __host__ __device__ static tv::array<int, 2> initial_offset(int thread_idx)   {
    ...
    int lane_row_offset = lane_idx / 4;
    int lane_col_offset = lane_idx % 4;
    ...
    int group_offset = group_idx * 8 * 2;
    // 0
    int row_offset = row_idx * 1 * 8; // 1d
  ...
  __forceinline__ __host__ __device__ static tv::array<int64_t, 3> iteration_inc_params(int stride)   {
    ...
    increments[0] = stride * 1 -
                    ...
                    stride * 8 * (1 - 1);
    increments[1] =
        stride * 1 - stride * 8 * (1 - 1);
    increments[2] = stride * 8;
    return increments;
  }
