...
#include <spconvlib/cumm/conv/main/Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8/inpitera/mask/Mask.h>
#include <spconvlib/cumm/conv/main/Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8/inpitera/gload/GlobalLoad.h>
#include <spconvlib/cumm/conv/main/Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8/inpitera/tmap/PitchLinearWarpRaked.h>
...
using Mask = spconvlib::cumm::conv::main::Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8::inpitera::mask::Mask;
using GlobalLoad = spconvlib::cumm::conv::main::Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8::inpitera::gload::GlobalLoad;
using ThreadMap = spconvlib::cumm::conv::main::Ampere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8::inpitera::tmap::PitchLinearWarpRaked;
...
using Params = spconvlib::cumm::conv::main::cpAmpere_s8s8s8s32f16tnt_m128n128k128m64n64k128A1T16832_300_C301LLL_SKD_S8::itera_p::SparseParams;
struct ForwardDgradSparseIOIterator {
  ...
  __forceinline__ __device__  ForwardDgradSparseIOIterator(Params & params, ConvProblem const& problem_size, const int8_t * ptr, int thread_id, const tv::array<int, 2>& threadblock_offset) : params_(params), problem_(problem_size), indice_ptr_(params.indice_ptr_)  {
    ...
    for (int s = 0; s < 8; ++s){
        ...
        for (int ss = 0; ss < 1; ++ss){
            TV_PRAGMA_UNROLL
            for (int v = 0; v < 1; ++v){
                uint32_t pred = (stride_offset_ + s * 4 + ss) < problem_.N;
                mask_[v] |= (pred << (s * 1 + ss));
            }
        }
    }
  ...
  __forceinline__ __device__ void load_with_pointer_offset(tv::array<int8_t, 128, 0>& frag, int32_t pointer_offset)   {
      ...
      for (int c = 0; c < 1; ++c){
        ...
        for (int ss = 0; ss < 1; ++ss){
          ...
          for (int v = 0; v < 1; ++v){
            ...
            //     tv::printf2(valid, s, access_pointer_num2 % 16, indice_offset, indice_offset%16, "AS", indices_[0], indices_[1], indices_[2], indices_[3], indices_[4], indices_[5], indices_[6], indices_[7], "A", blockIdx.x, blockIdx.y, blockIdx.z);
            // }
  ...
  __forceinline__ __device__ void load(tv::array<int8_t, 128, 0>& frag)   {
