...
#include <spconvlib/cumm/conv/main/Simt_f32f32f32f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK/output/out_ns_smem/tmap/Out5DLinear.h>
namespace spconvlib {
namespace cumm {
namespace conv {
namespace main {
...
using ThreadMap = spconvlib::cumm::conv::main::Simt_f32f32f32f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::output::out_ns_smem::tmap::Out5DLinear;
struct OutSmemLoader {
  ...
  __forceinline__ __device__  OutSmemLoader(float * ptr, int thread_idx)   {
    ...
    pointer_ = ptr + thread_offset[0] * 273 + thread_offset[1];
  }
        ...
        for (int group = 0; group < 1; ++group) {
            ...
            for (int row = 0; row < 1; ++row) {
                const float * cur_pointer =
                    pointer_ + row * 1 * 273 + group * 1 * 273 +
                    cluster * 1 * 273 + pointer_offset;
                int frag_row_idx =
