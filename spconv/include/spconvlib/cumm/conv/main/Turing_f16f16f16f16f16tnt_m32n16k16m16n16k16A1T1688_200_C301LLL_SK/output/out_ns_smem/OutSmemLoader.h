...
#include <spconvlib/cumm/conv/main/Turing_f16f16f16f16f16tnt_m32n16k16m16n16k16A1T1688_200_C301LLL_SK/output/out_ns_smem/tmap/Out5DLinear.h>
namespace spconvlib {
namespace cumm {
namespace conv {
namespace main {
...
using ThreadMap = spconvlib::cumm::conv::main::Turing_f16f16f16f16f16tnt_m32n16k16m16n16k16A1T1688_200_C301LLL_SK::output::out_ns_smem::tmap::Out5DLinear;
struct OutSmemLoader {
  ...
  __forceinline__ __device__  OutSmemLoader(tv::half_t * ptr, int thread_idx)   {
    ...
    pointer_ = ptr + thread_offset[0] * 24 + thread_offset[1];
  }
        ...
        for (int group = 0; group < 1; ++group) {
            ...
            for (int row = 0; row < 1; ++row) {
                const tv::half_t * cur_pointer =
                    pointer_ + row * 8 * 24 + group * 1 * 24 +
                    cluster * 1 * 24 + pointer_offset;
                int frag_row_idx =
