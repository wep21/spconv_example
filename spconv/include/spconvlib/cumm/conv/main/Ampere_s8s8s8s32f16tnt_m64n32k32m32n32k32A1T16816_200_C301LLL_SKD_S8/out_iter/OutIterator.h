...
#include <spconvlib/cumm/conv/main/cpAmpere_s8s8s8s32f16tnt_m64n32k32m32n32k32A1T16816_200_C301LLL_SKD_S8/out_params_ns/tmap/Out5DLinear.h>
...
using ThreadMap = spconvlib::cumm::conv::main::cpAmpere_s8s8s8s32f16tnt_m64n32k32m32n32k32A1T16816_200_C301LLL_SKD_S8::out_params_ns::tmap::Out5DLinear;
using Params = spconvlib::cumm::conv::main::cpAmpere_s8s8s8s32f16tnt_m64n32k32m32n32k32A1T16816_200_C301LLL_SKD_S8::out_params_ns::OutIteratorParams;
struct OutIterator {
  int8_t * pointer_;
  ...
  __forceinline__ __device__  OutIterator(Params const& params, int8_t * ptr, const int8_t * source_ptr, tv::array<int, 2> extent, tv::array<int, 2> offset_2d, int thread_idx) : params_(params)  {
    ...
    for (int c = 0; c < 1; ++c) {
        for (int v = 0; v < 1; ++v){
            column_masks_[c][v] = ((thread_offset[1] + 32 * c + v * 8) < extent[1]);
        }
    }
  ...
  __forceinline__ __device__ void store_with_offset(tv::array<int8_t, 8, 0> const & frag, int32_t offset)   {
    ...
    tv::alignedarray<int2, 1, 8> const *frag_ptr = reinterpret_cast<tv::alignedarray<int2, 1, 8> const *>(&frag);
    TV_PRAGMA_UNROLL
    for (int cluster = 0; cluster < 1; ++cluster){
      TV_PRAGMA_UNROLL
      for (int group = 0; group < 1; ++group){
        ...
        for (int row = 0; row < 1; ++row){
          ...
          tv::alignedarray<int2, 1, 8>  *memory_pointer =
              reinterpret_cast<tv::alignedarray<int2, 1, 8>  *>(cur_pointer + offset + indices_[frag_row_idx]);
          TV_PRAGMA_UNROLL
          for (int column = 0; column < 1; ++column){
            bool guard = row_guard && column_masks_[column][0];
            tv::gemm::global_store<tv::alignedarray<int2, 1, 8>, sizeof(tv::alignedarray<int2, 1, 8>)>(
                frag_ptr[frag_row_idx *  1 + column],
                (void *)&memory_pointer[column * 32 / 8],
                guard);
          }
        }
      }
        ...
        for (int row = 0; row < 1; ++row){
          ...
          for (int column = 0; column < 1; ++column){
            ...
            tv::gemm::global_load<tv::alignedarray<int2, 1, 8>, sizeof(tv::alignedarray<int2, 1, 8>)>(
                frag_ptr[frag_row_idx *  1 + column],
                (const void *)&memory_pointer[column * 32 / 8],
                guard);
          }
        }
      }
