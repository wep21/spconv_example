...
using Layout = spconvlib::cumm::conv::main::Ampere_f32f32f32f32f32tnt_m64n64k32m32n32k32A1T1688_200_C301LLL_SK::mma::mma_ns_wa::layout::MyTensorOpLayout;
struct SmemTileIterator {
  tv::array<float, 4> * pointer_[2];
  ...
  __forceinline__ __device__ tv::array<float, 4> * get(int s, int c)  const {
    tv::array<float, 4> * access_ptr = pointer_[s & 1];
  ...
  __forceinline__ __device__ void store_with_pointer_offset(tv::array<float, 16, 0> const& frag, int32_t pointer_offset)   {
    store_with_byte_offset(frag, pointer_offset * 32 / 8);
  }
  __forceinline__ __device__ void store_with_byte_offset(tv::array<float, 16, 0> const& frag, int32_t byte_offset)   {
  ...
  __forceinline__ __device__ void store(tv::array<float, 16, 0> const& frag)   {
