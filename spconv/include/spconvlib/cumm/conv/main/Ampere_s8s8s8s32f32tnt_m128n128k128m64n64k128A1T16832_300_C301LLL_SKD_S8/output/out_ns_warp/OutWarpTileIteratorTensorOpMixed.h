...
struct OutWarpTileIteratorTensorOpMixed {
  ...
  RowMajor layout_;
  int warp_column_;
  __forceinline__ __device__  OutWarpTileIteratorTensorOpMixed(int32_t * ptr, int warp_offset_m, int warp_offset_n, int lane_idx) : layout_(68), warp_column_(0)  {
    
    int tensorop_row = lane_idx / 4;
    int tensorop_col = lane_idx % 4;
    auto pointer = reinterpret_cast<tv::alignedarray<int2, 1, 8> *>(ptr) + tensorop_row * 68;
    TV_PRAGMA_UNROLL
    for (int i = 0; i < 2; ++i){
      // int swizzled_tensorop_col = (tensorop_col % 2) + ((
      //     (tensorop_col / 2) + i) % 2) * 2
  ...
  __forceinline__ __device__ void add_warp_offset(int warp_m, int warp_n)   {
    ...
    warp_column_ += warp_n * 64;
  }
  __forceinline__ __device__ void store_with_pointer_offset(tv::array<int32_t, 16, 0> const & frag, int32_t pointer_offset)   {
    ...
    for (int n = 0; n < 8; ++n){
      ...
      int ptr_idx = smem_line_offset % 2;
      auto ptr = pointers_[ptr_idx];
      int offset = n * 4 + pointer_offset / 2;
      ptr[offset] = frag_ptr[n];
    }
  }
  __forceinline__ __device__ void store(tv::array<int32_t, 16, 0> const& frag)   {
