...
#include <spconvlib/cumm/conv/main/Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK/inpiterb/gload/GlobalLoad.h>
#include <spconvlib/cumm/conv/main/Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK/inpiterb/mask/Mask.h>
#include <spconvlib/cumm/conv/main/Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK/inpiterb/tmap/PitchLinearWarpRaked.h>
...
using GlobalLoad = spconvlib::cumm::conv::main::Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK::inpiterb::gload::GlobalLoad;
using Mask = spconvlib::cumm::conv::main::Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK::inpiterb::mask::Mask;
using ThreadMap = spconvlib::cumm::conv::main::Ampere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK::inpiterb::tmap::PitchLinearWarpRaked;
...
using Params = spconvlib::cumm::conv::main::cpAmpere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK::iterb_p::WeightOptParams;
using Layout = spconvlib::cumm::conv::main::cpAmpere_f32f32f32f32f32tnt_m32n32k16m16n16k16A0T1688_200_C301LLL_SK::lb::TensorGeneric;
struct WeightIteratorDP4AV2Mask {
  ...
  Mask mask_backup_;
  Mask mask_;
  __forceinline__ __device__  WeightIteratorDP4AV2Mask(Params const& params, ConvProblem const& problem_size, const float * ptr, int thread_id, const tv::array<int, 2>& threadblock_offset) : params_(params), problem_size_(problem_size), pointer_(reinterpret_cast<const char *>(ptr))  {
    ...
    pointer_ += (thread_offset[0] * params.layout.strides[0] + thread_offset[1]) * 32 / 8;
    mask_backup_ = mask_;
  }
  ...
  __forceinline__ __device__ const tv::array<float, 1> * get(int stride, int contig, int ss)  const {
    
    return reinterpret_cast<const tv::array<float, 1> *>(pointer_ + contig * 16 * 32 / 8);
  }
  __forceinline__ __device__ void load_with_pointer_offset(tv::array<float, 4, 0>& frag, int32_t pointer_offset)   {
    
    frag.clear();
    tv::array<float, 1> *frag_ptr = reinterpret_cast<tv::array<float, 1> *>(&frag);
    TV_PRAGMA_UNROLL
    for (int s = 0; s < 1; ++s){
      TV_PRAGMA_UNROLL
      for (int c = 0; c < 1; ++c){
        ...
        for (int ss = 0; ss < 1; ++ss){
          tv::array<float, 1> const *access_ptr = get(s, c, ss) + pointer_offset / 1;
          TV_PRAGMA_UNROLL
          for (int v = 0; v < 4; ++v){
            int idx = s * 4 + 
                c * 4 + ss * 4 + v;
            // tv::array<float, 1> const *access_ptr = get(s, c, ss) + v + pointer_offset / 1;
            // tv::gemm::global_load<tv::array<float, 1>, sizeof(tv::array<float, 1>)>(
