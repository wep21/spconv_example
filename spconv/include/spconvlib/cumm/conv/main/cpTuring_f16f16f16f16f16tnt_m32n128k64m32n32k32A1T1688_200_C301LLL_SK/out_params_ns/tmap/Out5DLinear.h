...
struct Out5DLinear {
  __forceinline__ __host__ __device__ static tv::array<int, 2> initial_offset(int thread_idx)   {
    ...
    int group_idx = residual_cluster / 8;
    int residual_group = residual_cluster % 8;
    ...
    int cluster_offset = cluster_idx * 1 * 1 *
                        8 * 4;
    ...
    return {cluster_offset + group_offset + row_offset + lane_row_offset, 
        (column_offset + lane_col_offset) * 4};
  }
