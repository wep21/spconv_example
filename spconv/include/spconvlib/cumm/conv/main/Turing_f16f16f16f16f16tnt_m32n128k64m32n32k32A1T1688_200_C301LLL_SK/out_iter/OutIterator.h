...
#include <spconvlib/cumm/conv/main/cpTuring_f16f16f16f16f16tnt_m32n128k64m32n32k32A1T1688_200_C301LLL_SK/out_params_ns/tmap/Out5DLinear.h>
...
using ThreadMap = spconvlib::cumm::conv::main::cpTuring_f16f16f16f16f16tnt_m32n128k64m32n32k32A1T1688_200_C301LLL_SK::out_params_ns::tmap::Out5DLinear;
using Params = spconvlib::cumm::conv::main::cpTuring_f16f16f16f16f16tnt_m32n128k64m32n32k32A1T1688_200_C301LLL_SK::out_params_ns::OutIteratorParams;
struct OutIterator {
  tv::half_t * pointer_;
  ...
  __forceinline__ __device__  OutIterator(Params const& params, tv::half_t * ptr, tv::array<int, 2> extent, tv::array<int, 2> offset_2d, int thread_idx) : params_(params)  {
    ...
    for (int c = 0; c < 1; ++c) {
        for (int v = 0; v < 1; ++v){
            column_masks_[c][v] = ((thread_offset[1] + 128 * c + v * 4) < extent[1]);
        }
    }
  ...
  __forceinline__ __device__ void store_with_offset(tv::array<tv::half_t, 4, 0> const & frag, int32_t offset)   {
        ...
        for (int row = 0; row < 1; ++row){
          ...
          for (int column = 0; column < 1; ++column){
            ...
            tv::gemm::global_store<tv::alignedarray<int2, 1, 8>, sizeof(tv::alignedarray<int2, 1, 8>)>(
                ...
                (void *)&memory_pointer[column * 128 / 4],
                guard);
          }
        }
      }
  ...
  __forceinline__ __device__ void load_with_offset(tv::array<tv::half_t, 4, 0>  & frag, int32_t offset)   {
        ...
        for (int row = 0; row < 1; ++row){
          ...
          for (int column = 0; column < 1; ++column){
            ...
            tv::gemm::global_load<tv::alignedarray<int2, 1, 8>, sizeof(tv::alignedarray<int2, 1, 8>)>(
                frag_ptr[frag_row_idx *  1 + column],
                (const void *)&memory_pointer[column * 128 / 4],
                guard);
          }
        }
      }
