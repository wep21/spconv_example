...
struct Out5DLinear {
  __forceinline__ __host__ __device__ static tv::array<int, 2> initial_offset(int thread_idx)   {
    ...
    int cluster_idx = warp_idx / 2;
    int residual_cluster = warp_idx % 2;
    int group_idx = residual_cluster / 1;
    int residual_group = residual_cluster % 1;
    int row_idx = residual_group / 1;
    int col_idx = residual_group % 1;
    // Compute per-lane offset
    // in 1d warp, row offset always 0
    int lane_row_offset = lane_idx / 32;
    int lane_col_offset = lane_idx % 32;
    ...
    int cluster_offset = cluster_idx * 4 * 1 *
                        1 * 8;
    // 0, 1, 0, 1 * 8// warp 0 handle [0, 2], warp 1 handle [1, 3]
    int group_offset = group_idx * 1 * 8;
    // 0
    int row_offset = row_idx * 1 * 1; // 1d
    ...
    return {cluster_offset + group_offset + row_offset + lane_row_offset, 
        (column_offset + lane_col_offset) * 1};
  }
  __forceinline__ __host__ __device__ static tv::array<int64_t, 3> iteration_inc_params(int stride)   {
    ...
    increments[0] = stride * 1 -
                    ...
                    stride * 1 * (1 - 1);
    increments[1] =
        stride * 16 - stride * 1 * (1 - 1);
    increments[2] = stride * 1;
    return increments;
  }
