...
struct OutWarpTileIteratorTensorOpMixed {
  ...
  RowMajor layout_;
  int warp_column_;
  __forceinline__ __device__  OutWarpTileIteratorTensorOpMixed(float * ptr, int warp_offset_m, int warp_offset_n, int lane_idx) : layout_(12), warp_column_(0)  {
    
    int tensorop_row = lane_idx / 4;
    int tensorop_col = lane_idx % 4;
    auto pointer = reinterpret_cast<tv::alignedarray<int2, 1, 8> *>(ptr) + tensorop_row * 12;
    TV_PRAGMA_UNROLL
    for (int i = 0; i < 1; ++i){
