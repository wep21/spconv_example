...
using Params = spconvlib::cumm::conv::main::cpTuring_s8s8s8s32f16tnt_m32n32k32m32n16k32A1T8816_200_C301LLL_SKD_S8::out_params_scalebias_ns::OutIteratorParams;
struct OutIterator {
  ...
  __forceinline__ __device__  OutIterator(Params const& params, const tv::half_t * ptr, tv::array<int, 2> extent, tv::array<int, 2> offset_2d, int thread_idx) : params_(params)  {
    ...
    for (int c = 0; c < 1; ++c) {
        for (int v = 0; v < 2; ++v){
            column_masks_[c][v] = ((thread_offset[1] + 32 * c + v * 2) < extent[1]);
        }
    }
