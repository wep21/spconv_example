...
#include <spconvlib/cumm/conv/main/Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK/inpitera/mask/Mask.h>
#include <spconvlib/cumm/conv/main/Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK/inpitera/gload/GlobalLoad.h>
#include <spconvlib/cumm/conv/main/Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK/inpitera/tmap/PitchLinear.h>
...
using Mask = spconvlib::cumm::conv::main::Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::inpitera::mask::Mask;
using GlobalLoad = spconvlib::cumm::conv::main::Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::inpitera::gload::GlobalLoad;
using ThreadMap = spconvlib::cumm::conv::main::Simt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::inpitera::tmap::PitchLinear;
using ConvProblem = spconvlib::cumm::conv::main::cpSimt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::cp::ConvProblem;
using Params = spconvlib::cumm::conv::main::cpSimt_f16f16f16f32f32tnt_m32n256k8m32n64k8A1_200_C301LLL_SK::itera_p::SparseParams;
struct ForwardDgradSparseIOIterator {
  ...
  __forceinline__ __device__ const tv::alignedarray<tv::half_t, 1, 2> * get(int indice_offset)  const {
    
    return reinterpret_cast<const tv::alignedarray<tv::half_t, 1, 2> *>( pointer_ + indice_offset);
  }
  __forceinline__ __device__ void load_with_pointer_offset(tv::array<tv::half_t, 2, 0>& frag, int32_t pointer_offset)   {
      ...
      for (int c = 0; c < 1; ++c){
        ...
        for (int ss = 0; ss < 1; ++ss){
          ...
          for (int v = 0; v < 1; ++v){
            ...
            auto access_pointer = reinterpret_cast<const tv::alignedarray<tv::half_t, 1, 2> *>(pointer_ + indice_offset + 
                c * 2) + v;
            ...
            //     tv::printf2(valid, s, access_pointer_num2 % 16, indice_offset, indice_offset%16, "AS", indices_[0], indices_[1], "A", blockIdx.x, blockIdx.y, blockIdx.z);
            // }
  ...
  __forceinline__ __device__ void load(tv::array<tv::half_t, 2, 0>& frag)   {
