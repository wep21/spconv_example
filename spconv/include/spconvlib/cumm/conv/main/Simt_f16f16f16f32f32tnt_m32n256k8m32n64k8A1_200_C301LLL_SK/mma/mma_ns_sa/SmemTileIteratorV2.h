...
struct SmemTileIteratorV2 {
  ...
  __forceinline__ __device__  SmemTileIteratorV2(int stride, tv::half_t* ptr, int thread_id)   {
    ...
    int offset = (thread_offset[1] / 1) * 40
         + thread_offset[0] * 1;
    ...
    // if (false) {
    //     inc_advance_ = 32 * sizeof(tv::half_t);
    // } else {
  ...
  __forceinline__ __device__ void store_with_pointer_offset(tv::array<tv::half_t, 2, 0> const & frag, int32_t pointer_offset)   {
